...
if option == "Wydźwięk emocjonalny tekstu (eng)":
    ...
    text = st.text_area(label="Wpisz tekst do przetłumaczenia EN -> GER")
    if st.button("Przetłumacz na GER:"):
        if not text.strip():
            ...
            with st.spinner(text="Tłumaczę..."):
                try:
                    ...
                    ger = out[0]["translation_text"]
                    time.sleep(0.5)
                    st.success("Przetłumaczone!")
                    st.markdown("Wprowadzony tekst po niemiecku: ")
                    st.info(ger)
                    st.toast("Przetłumaczony tekst jest gotowy!")
                except ValueError as e:
                    st.error("Tekst jest zbyt długi! Podziel go na mniejsze części.")
                except Exception as e:
                    st.error("O nie! Coś nie zadziałało.")


st.subheader('Zadanie do wykonania')
st.write(
    'Wykorzystaj Huggin Face do stworzenia swojej własnej aplikacji tłumaczącej tekst z języka angielskiego na język niemiecki. Zmodyfikuj powyższy kod dodając do niego kolejną opcję, tj. tłumaczenie tekstu. Informacje potrzebne do zmodyfikowania kodu znajdziesz na stronie Huggin Face - https://huggingface.co/docs/transformers/index')
